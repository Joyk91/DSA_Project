...
str(x, vec.len=2)
x[x[,"zeroVar"] > 0, ]  # ethanol gives us no information with respect to growth/nogrowth we can remove if we want  

x[x[,"zeroVar"] + x[,"nzv"] > 0, ] 
# ethanol is the only predictor that gives us no information gain 
df$ethanol <- NULL 
